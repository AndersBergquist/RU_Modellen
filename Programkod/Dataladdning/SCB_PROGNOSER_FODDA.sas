proc  ds2;
	package &prglib..rumprg_SCB_prog_fodda / overwrite=yes;
		...
		method rumprg_SCB_prog_fodda();
			arkivUrl='http://api.scb.se/OV0104/v1/doris/sv/ssd/START/BE/BE0401/BE0401B';
			inUrl='http://api.scb.se/OV0104/v1/doris/sv/ssd/START/BE/BE0401/BE0401B/BefProgFoddaMedel';
			aktUrl='http://api.scb.se/OV0104/v1/doris/sv/ssd/START/BE/BE0401/BE0401A/BefProgFoddaMedelN';
			maxCeller=50000;
			datalib=%tslit(&datalib);
			dataTabell='SCB_PROGNOSER_FODDA';
			startBasAr=2010;
		end;
		method run();
		...
		method uppdateraTabell(integer iBasar, varchar(32) iTabellNamnTmp);
			...
			do while(s_sqlVar.fetch([values, valuetexts])=0);
				if valuetexts='Födda' then rowValues='fodda';
			...
			if upcase(strip(iTabellNamnTmp)) = 'BEFPROGFODDAMEDELN' then foddland='MODERFODLANDGRUPP_NM';
